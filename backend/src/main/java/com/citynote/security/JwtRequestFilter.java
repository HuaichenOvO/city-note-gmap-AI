...
public class JwtRequestFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        if (requestURI.equals("/api/auth/login") || requestURI.equals("/api/auth/register")) {
            System.out.println("[JWT Filter] Allowing auth endpoint: " + requestURI);
        ...
        if (requestURI.equals("/api/upload/image") && method.equals("POST")) {
            System.out.println("[JWT Filter] Allowing file upload endpoint: " + requestURI);
            chain.doFilter(request, response);
            return;
        }
        ...
        if (requestURI.startsWith("/api/upload/image/") && method.equals("GET")) {
            System.out.println("[JWT Filter] Allowing image access endpoint: " + requestURI);
            chain.doFilter(request, response);
            return;
        }
        // For /api/auth/me, we need to check JWT but not block if missing
        if (requestURI.equals("/api/auth/me")) {
            System.out.println("[JWT Filter] Processing /api/auth/me endpoint");
            // Continue to JWT processing below
        }

        // Allow GET requests to public event endpoints (but not user-specific or
        // can-modify)
        if (requestURI.startsWith("/api/event/") && method.equals("GET") &&
                !requestURI.contains("/can-modify") && !requestURI.contains("/user/")) {
            System.out.println("[JWT Filter] Allowing public GET event endpoint: " + requestURI);
            chain.doFilter(request, response);
            return;
        }
        ...
        if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            try {
                ...
                if (jwtTokenUtil.validateToken(jwt, userDetails)) {
                    ...
                    System.out.println("[JWT Filter] Invalid JWT token for user: " + username);
                }
            } catch (Exception e) {
